...
class DTSDataset(Dataset):
  ...
  def __init__(self,
    ...
    if tile_id is not None:
      tmp = []
      row = self.df_raw.iloc[tile_id].copy()
      df_speed = pd.read_csv(row['label_speed'])
      for day in range(7):
      ...
      for index, row in self.df_raw.iterrows():
        ...
        for day in days:
          for hour in hours:
            times = np.unique(df_speed[['dow', 'hour']].values, axis=0).tolist()
            if [day, hour] in times:
              row['time'] = [day, hour]
              tmp.append(row.copy())
      self.df = pd.concat(tmp, axis=1, ignore_index=True).T
